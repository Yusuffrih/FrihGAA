    ...
    if size:
        if item_id in list(basket.keys()):
            if size in basket[item_id]['items_by_size'].keys():
                ...
                messages.success(
                    request, (f"You've updated size {size.upper()} {product.name} \
                        quantity to {basket[item_id]['items_by_size'][size]}")
                    )
            else:
                basket[item_id]['items_by_size'][size] = quantity
                messages.success(
                    request, (f"You've added size {size.upper()} {product.name} \
                        to your basket")
                    )
        else:
            basket[item_id] = {'items_by_size': {size: quantity}}
        ...
        if item_id in list(basket.keys()):
            ...
            messages.success(
                request, (f"You've updated {product.name} \
                    quantity to {basket[item_id]}")
                )
        else:
            basket[item_id] = quantity
            messages.success(
                request, (f"You've added {product.name} \
                    from your basket")
                )
...
def adjust_basket(request, item_id):
    ...
    if size:
        if quantity > 0:
            basket[item_id]['items_by_size'][size] = quantity
            messages.success(
                request, (f"You've updated size {size.upper()} \
            ...
            if not basket[item_id]['items_by_size']:
                basket.pop(item_id)
                messages.success(
                    request, (f"You've removed size {size.upper()} \
        ...
        if quantity > 0:
            ...
            messages.success(
                request,
                (f"You've removed {product.name} from your basket")
                )
    ...
    try:
        ...
        if size:
            ...
            if not basket[item_id]['items_by_size']:
                ...
                messages.success(
                    request,
                    (f"You've removed size {size.upper()} {product.name} \
                        from your basket")
                    )
        else:
            basket.pop(item_id)
